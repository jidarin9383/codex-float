...
enum Smoke {
    ...
    private static func mathAndFixtures() throws {
        ...
        try expect(QuotaAttention.from(remainingPercent: 51) == .healthy, "51 healthy")
        try expect(QuotaAttention.from(remainingPercent: 50) == .attention, "50 attention")
        try expect(QuotaAttention.from(remainingPercent: 21) == .attention, "21 attention")
        try expect(QuotaAttention.from(remainingPercent: 20) == .critical, "20 critical")
        try expect(QuotaAttention.from(remainingPercent: 18) == .critical, "18 critical")
